...
public class CustomProgressBar extends ProgressBar {
    ...
    public CustomProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        //获取属性
        TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.CustomProgressBar, defStyleAttr, 0);
        //获取圆的颜色，默认黑色
        mCircleColor = typedArray.getColor(R.styleable.CustomProgressBar_circleColor, Color.BLACK);
        //获取圆的粗细，默认5dp
        mCircleWidth = (int) typedArray.getDimension(R.styleable.CustomProgressBar_circleWidth, ImageUtils.dp2px(context, 5));
        //获取圆的起始角度，默认0度
        mStartAngle = typedArray.getInteger(R.styleable.CustomProgressBar_startAngle, 0);
        //获取文字大小，默认18sp
        mTextSize = (int) typedArray.getDimension(R.styleable.CustomProgressBar_textSize, ImageUtils.sp2px(getContext(), 18));
        //获取文字颜色，默认黑色
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode == MeasureSpec.AT_MOST) {
            width = getPaddingLeft() + getPaddingRight() + ImageUtils.dp2px(getContext(), 60);
        }
        if (heightMode == MeasureSpec.AT_MOST) {
            height = getPaddingTop() + getPaddingBottom() + ImageUtils.dp2px(getContext(), 60);
        }
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        mPaint.setStrokeWidth(ImageUtils.dp2px(getContext(), 1));
        ...
        canvas.drawText(strProgress,
                ...
                (getHeight() - getPaddingTop() - getPaddingBottom() - mBounds.height()) / 2 + getPaddingTop() + mBounds.height(),
                mPaint);
    }
}
